...
def setup_vectorstore():
    ...
    """
    embeddings = get_embeddings()                  # 1. embedding model
    text = load_knowledge_base()                   # 2. load data/knowledge_base.txt
    chunks = split_text(text, chunk_size=500, chunk_overlap=50)  # 3. split
    print(f"📚 Đã chia thành {len(chunks)} chunks")
    vectorstore = build_vectorstore(chunks, embeddings)  # 4. FAISS index
    return vectorstore
